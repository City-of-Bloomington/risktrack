 ...
 * 
 */
@WebServlet(urlPatterns = {"/SearchAutoAccidentServ","/SearchAutoAccident"})
public class SearchAutoAccidentServ extends TopServlet{

    static Logger logger = LogManager.getLogger(SearchAutoAccidentServ.class);
    /**
     * search the auto accidents 
     *
	...
	out.println("<h3 class=\"titleBar\">Search Auto Accidents </h3>");
	if(success){
	    if(!message.equals(""))
	...
	out.println("<table>");
	out.println("<tr><td><label for=\"id\">Auto Accident ID:</label>");
	...
	out.println("<label for=\"autoYear\">Year:</label>"+
		    ...
		    autoYear+"\" size=\"4\" maxlength=\"4\" />");
	out.println("<label for=\"owner\">Owner:</label>"+
	...
	Enumeration values = req.getParameterNames();
	String [] vals;
	AutoAccidentList dl = new AutoAccidentList(debug);
	while (values.hasMoreElements()){
	    name = ((String)values.nextElement()).trim();
	    vals = req.getParameterValues(name);
	    value = vals[vals.length-1].trim();	
	...
	out.println("<div id=\"mainContent\">");
	List<AutoAccident> accidents = null;

	String msg = dl.lookFor();
	if(msg.equals("")){
	    accidents = dl.getAutoAccidents();
	}
	else{
	    message += msg;
	    success = false;
	...
	if(accidents != null){
	    ncnt = accidents.size();
	    if(ncnt == 1){
		AutoAccident one = accidents.get(0);
		res.sendRedirect(url+"AutoAccident?id="+one.getId());
		return;
	    }	
	...
	if(ncnt > 0){
		...
		AutoAccident one = accidents.get(i);
		out.println("<tr>");
		// 
		out.println("<td><a href=\""+url+"AutoAccident?"+
		...
		if(show[2]){
			...
			MiscAccidentType dt = new MiscAccidentType(debug, str);
			if(dt != null){
			    str = dt.getType();
		...
		//
		if(show[3]){
		    str = one.getAccidDate();
		    if(str.equals("")) str = "&nbsp;";
		    out.println("<td>"+str+"</td>");
		}
		if(show[4]){
		    str = one.getAccidLocation();
		    if(str.equals("")) str = "&nbsp;";
		    out.println("<td>"+str+"</td>");
		}
		if(show[5]){
		...
		if(show[6]){
		    str = one.getEmpInjured();
		    if(str.equals("")) str = "&nbsp;";
		    out.println("<td>"+str+"</td>");
		}
		if(show[7]){
		    Department dp = one.getDepartment();
		    if(dp == null) str = "&nbsp;";
		    else
			str = dp.getName();
		    out.println("<td>"+str+"</td>");
