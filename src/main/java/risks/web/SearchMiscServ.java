 ...
 * 
 */
@WebServlet(urlPatterns = {"/SearchMiscServ","/SearchMisc"})
public class SearchMiscServ extends TopServlet{

    static Logger logger = LogManager.getLogger(SearchMiscServ.class);
    /**
     * search the misc accidents 
     *
	...
	out.println("<h3 class=\"titleBar\">Search Misc Accidents </h3>");
	if(success){
	    if(!message.equals(""))
	...
	out.println("<table>");
	out.println("<tr><td><label for=\"id\">Misc Accident ID:</label>");
	...
	Enumeration values = req.getParameterNames();
	String [] vals;
	MiscAccidentList dl = new MiscAccidentList(debug);
	while (values.hasMoreElements()){
	    name = ((String)values.nextElement()).trim();
	    vals = req.getParameterValues(name);
	    value = vals[vals.length-1].trim();	
	...
	out.println("<div id=\"mainContent\">");
	List<MiscAccident> accidents = null;

	String msg = dl.lookFor();
	if(msg.equals("")){
	    accidents = dl.getMiscAccidents();
	}
	else{
	    message += msg;
	    success = false;
	...
	if(accidents != null){
	    ncnt = accidents.size();
	    if(ncnt == 1){
		MiscAccident one = accidents.get(0);
		res.sendRedirect(url+"MiscAccident?id="+one.getId());
		return;
	    }	
	...
	if(ncnt > 0){
		...
		MiscAccident one = accidents.get(i);
		out.println("<tr>");
		// 
		out.println("<td><a href=\""+url+"MiscAccident?"+
