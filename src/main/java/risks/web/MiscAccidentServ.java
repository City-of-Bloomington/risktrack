 ...
 *
 */
@WebServlet(urlPatterns = {"/MiscAccidentServ","/MiscAccident"})
public class MiscAccidentServ extends TopServlet{

    static Logger logger = LogManager.getLogger(MiscAccidentServ.class);
	...
	MiscAccident ds = new MiscAccident(debug);
	...
	if(action.equals("Save")){
		...
		if(!relatedId.equals("")){
		    Related rel = new Related(id,relatedId,"Misc Accidents", debug);
		    back = rel.doSave();
		    if(!back.equals("")){
			message += back;
			logger.error(back);
	...
	out.println("<h3 class=\"titleBar\">Miscellaneous Accident "+id+"</h3>");
	//
	...
	if(id.equals("")){
	    ...
	    out.println("<input name=\"action\" "+
			...
			url+
			"GetEmpInfoServ?legalType=MiscAccident"+
			"&opener=MiscAccidentSer"+
			"&relatedId="+id+
		...
		for(Employee empo: ls){
		    Helper.printEmployee(out,empo,url,id,"MiscAccidentServ");
		}
	    }
	    out.println("</table>");			
	    out.println("</fieldset>");	
	...
	if(!subToInsur.equals("")){
		...
		out.println("<input name=\"action\" "+
			    ...
			    "tabindex=\"33\" onClick=\"window.open('"+
			    url+
			    "InsuranceServ?legalType=MiscAccident"+
			    "&opener=MiscAccidentServ"+
		...
		if(ls != null){
			...
			Helper.printInsurance(out,insr,url,id,"MiscAccident","MiscAccidentSer");
		    }
		    out.println("</table>");
		}
		out.println("</fieldset>");					
	...
	if(action.startsWith("Print")){
		...
		out.println("<input type=\"button\" value=\"Add Notes\" "+
			    "onclick=\"window.open('" + url + 
			    "NoteServ?risk_id="+id+"&opener=MiscAccidentServ','Notes',"+
