 ...
 *
 */
@WebServlet(urlPatterns = {"/DisasterServ","/Disaster"})
public class DisasterServ extends TopServlet{
	...
	String  
	    type="", id="", status="", insur_id="",
	    empid="", userid="", supervisor="", empInjured="",
	    empName="", deptPhone="", dept="",
	    vin="", empPhone="",
	...
	Disaster ds = new Disaster(debug);
	...
	while (values.hasMoreElements()){
	    ...
	    else if(name.equals("delAutoId")){
		delAutoId = vals; // array
	    }
	...
	if(action.equals("Save")){
		...
		if(!relatedId.equals("")){
		    Related rel = new Related(id,relatedId,"Natural Disaster Accident", debug);
		    back = rel.doSave();
		    if(!back.equals("")){
			message += back;
			logger.error(back);
	    ...
	    if(delAutoId != null){
		for(int i=0;i<delAutoId.length;i++){
		    String aid = delAutoId[i];
	...
	out.println("<h3 class=\"titleBar\">Natural Disaster Accident "+id+"</h3>");
	//
	...
	out.println("<label for=\"subToInsur\">Submitted to Insurance? "+
		    "</label>"+
		    "<input name=\"subToInsur\" id=\"subToInsur\" "+
		    "onchange=\"doRefresh()\" "+
		    "value=\"y\" type=\"checkbox\" "+subToInsur+" />Yes.");
	out.println("</td></tr>");
	//
	...
	if(departments != null && departments.size() > 0){
		...
		if(mainDept.getId().equals(dpp.getId()))
		    out.println("<option selected=\"selected\" value=\""+dpp.getId()+"\">"+dpp.getName()+"</option>");
		else
		    out.println("<option value=\""+dpp.getId()+"\">"+dpp.getName()+"</option>");
	    }
	}
	...
	if(id.equals("")){
	    ...
	    out.println("<input name=\"action\" "+
			...
			url+
			"GetEmpInfoServ?legalType=Disaster"+
			"&relatedId="+id+
	    ...
	    List<Employee> ls = ds.getEmployees();
	    if(ls != null){
		for(Employee empo: ls){
		    Helper.printEmployee(out,empo,url,id,"DisasterServ");
		}
	    }
	    out.println("</table>");			
	    out.println("</fieldset>");	
	...
	if(!autoDamage.equals("")){
	    ...
	    if(!id.equals("")){
		String str="", all="";
		List<Auto> list = ds.getVehicles();
		if(list != null && list.size() > 0){
		    for(Auto auto:list){
			str = auto.getId();
			all += "<tr><td><input type=\"checkbox\" "+
			    "name=\"delAutoId\" value=\""+str+"\">**</td>";
			str = auto.getVin();
			all += "<td>"+str+"</td>";
	...
	if(!propDamage.equals("")){
	    ...
	    out.println("<input name=\"propPaid\" id=\"propPaid\" value=\"y"+
			"\" type=\"checkbox\" "+propPaid+" /> <label>Paid</label>");
	...
	if(!subToInsur.equals("")){
		...
		out.println("<input name=\"action\" "+
			    ...
			    "tabindex=\"33\" onClick=\"window.open('"+
			    url+
			    "InsuranceServ?legalType=Disaster"+
		...
		if(ls != null){
			...
			Helper.printInsurance(out,insr,url,id,"Disaster","DisasterServ");
		    }
		    out.println("</table>");
		}
		out.println("</fieldset>");					
	...
	if(action.startsWith("Print")){
		...
		out.println("<input type=\"button\" value=\"Add Notes\" "+
			    "onclick=\"window.open('" + url + 
			    "NoteServ?risk_id="+id+"&opener=DisasterServ','Notes',"+
