...
public class AutoAccident extends Risk{

    static Logger logger = LogManager.getLogger(AutoAccident.class);
    String type="", 
	status="", dept_id="",
    ...
    List<RiskPerson> claimants = null;
    //
    // basic constructor
    public AutoAccident(boolean deb){

	debug = deb;

    }
    public AutoAccident(boolean deb, String _id){
    ...
    public boolean hasClaimants(){
	getClaimants();
	return claimants != null && claimants.size() > 0;
    }		
	...
	if (o instanceof AutoAccident) {
	    AutoAccident c = (AutoAccident) o;
	...
	try{
	    ...
	    pstmt.setString(1, "autoAccident");
	    pstmt.executeUpdate();
	    //
	    ...
	    qq = "insert into auto_accidents values "+
		"(?,?,?,?,?, "+
		"?,?,?,?,?,"+
		"?,?,?,?,?,"+
		"?,?,?,?,?,"+
	...
	String str="", msg="", back="";
	String qq = "";
	qq = "update auto_accidents set type=?,status=?,dept_id=?,"+
	...
	String qq4 = "delete from auto_accidents where id=?";
	if(debug){
	    logger.debug(qq);
	}
	con = Helper.getConnection();
