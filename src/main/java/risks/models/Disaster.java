...
public class Disaster extends Risk{

    static Logger logger = LogManager.getLogger(Disaster.class);
    String type="", 
	status="", dept_id="",
    ...
    String damage="", autoDamage="", insur_id="", 
	totalCost="";
    String propDamage="",
	totalCostP="", 
    ...
    //
    // basic constructor
    public Disaster(boolean deb){

	debug = deb;

    }
    public Disaster(boolean deb, String _id){
    ...
    //
    // prob not needed
    public void setInsur_id(String val){
	if(val != null)
	    insur_id = val;
    }		
    ...
    public void setVehicles(List<Auto> val){
	if(val != null)
	    vehicles = val;
    }
	...
	if(employees == null && !id.equals("")){
	    ...
	    el.lookFor();
	    List<Employee> emps = el.getEmployees();
	...
	if(vehicles == null && !id.equals("")){
	    AutoList al = new AutoList(debug, id);
	    al.lookFor();
	    List<Auto> ones = al.getAutos();
	...
	try{
		...
		if(!back.equals("")){
		    ...
		    insur_id = insurance.getId();
		}
	    }
	    qq = "insert into risk_sequences values(0,'disaster')";
	    if(debug){
		logger.debug(qq);
	    }
	    ...
	    qq = "insert into disasters values (?,?,?,?,?, ?,?,?,?,?,"+
		"?,?,?,?,?, ?,?,?,?,?,"+
		"?,?,?,?,?, ?,?,?)";
	    if(debug){
		logger.debug(qq);
	    ...
	    if(type.equals(""))
		stmt3.setNull(jj++, Types.INTEGER);
	    else
		stmt3.setString(jj++, type);
	    ...
	    if(deptPhone.equals("")){
		stmt3.setNull(jj++, Types.VARCHAR);
	    }
	    else {
		stmt3.setString(jj++, deptPhone);
	    }
	    if(accidDate.equals(""))
		stmt3.setNull(jj++, Types.DATE);
	    else{
		...
		stmt3.setString(jj++, accidLocation);
	    }
	    if(empInjured.equals("")){
		stmt3.setNull(jj++, Types.CHAR);
	    }
	    else {
		stmt3.setString(jj++, "y");
	    }
	    if(damage.equals("")){
		stmt3.setNull(jj++, Types.VARCHAR);
	    }
	    else {
		stmt3.setString(jj++, damage);
	    }
	    if(totalCost.equals("")){
		stmt3.setString(jj++, "0");
	    }
	    else {
		stmt3.setString(jj++, totalCost);
	    }
		...
		stmt3.setString(jj++, whatProp);
	    }
	    if(repairInfo.equals("")){
		stmt3.setNull(jj++, Types.VARCHAR);
	    }
	    else {
		stmt3.setString(jj++, repairInfo);
	    }
	    if(otherType.equals("")){
		stmt3.setNull(jj++, Types.VARCHAR);
	    }
	    else {
		stmt3.setString(jj++, otherType);
	    }
	    if(paidByCity.equals("")){
		stmt3.setString(jj++, "0");
	    }
	    else {
		stmt3.setString(jj++, paidByCity);
	    }
	    if(paidByInsur.equals("")){
		stmt3.setString(jj++, "0");
	    }
	    else {
		stmt3.setString(jj++, paidByInsur);
	    }
	    if(miscByCity.equals("")){
		stmt3.setString(jj++, "0");
	    }
	    else {
		stmt3.setString(jj++, miscByCity);
	    }
	    if(paidByRisk.equals("")){
		stmt3.setString(jj++, "0");
	    }
	    else {
		stmt3.setString(jj++, paidByRisk);
	    }
	    if(insur_id.equals(""))
		stmt3.setNull(jj++, Types.INTEGER);
	    else
		stmt3.setString(jj++, insur_id);
	    if(deptContact.equals("")){
		stmt3.setNull(jj++, Types.VARCHAR);
	    }
	    else {
		stmt3.setString(jj++, deptContact);
	    }
	    stmt3.executeUpdate();
	    //
		...
		qq = "insert into disaster_emps values("+employee.getId()+","+id+")";
		if(debug){
		    logger.debug(qq);
		}
  ...
  public  String doUpdate(){
	...
	String str="", msg="", back="";
	String qq = "";
	qq = "update disasters set type=?, status=?, dept_id=?, "+
	    ...
	    " otherType=?,paidByCity=?,paidByInsur=?,miscByCity=?,"+
	    " paidByRisk=?,insur_id=?,deptContact=? "+
	    " where id=? ";
	...
	try{
		...
		stmt.setString(jj++, insur_id);
	    if(deptContact.equals("")){
		stmt.setNull(jj++, Types.VARCHAR);
	    }
	    else {
		stmt.setString(jj++, deptContact);
	    }
		...
		if(!back.equals("")){
		    ...
		    qq = "insert into disaster_emps values(?,?)";
		    stmt2 = con.prepareStatement(qq);
	...
	String qq3 = "delete from disaster_emps where risk_id=?";
	String qq4 = "delete from disasters where id=?";		
	if(debug){
	    logger.debug(qq);
	}
	con = Helper.getConnection();
	...
	try{
	    if(!insur_id.equals("")){
		insurance = new Insurance(debug, insur_id);
	    }
